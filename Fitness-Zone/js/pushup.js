...
function checkReps(pose) {
  ...
  if ( ( leftSEWAngle >= 0.9 * ANGLE_CHECK && leftShoulder.confidence > 0.1 && leftElbow.confidence > 0.1 && leftWrist.confidence > 0.1) || 
    (rightSEWAngle >= 0.9 * ANGLE_CHECK && rightShoulder.confidence > 0.1 && rightElbow.confidence > 0.1 && rightWrist.confidence > 0.1))   
      {
    if (!start_frame) {
  ...
  if (Math.abs(leftEWVec.y) < 30 && Math.abs(rightEWVec.y) < 30) {
